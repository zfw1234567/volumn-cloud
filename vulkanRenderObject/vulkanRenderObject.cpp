...
vulkanRenderObject::vulkanRenderObject(VkDevice device, VkPhysicalDevice physicalDevice, VkExtent2D swapChainExtent, VkCommandPool commandPool,VkQueue graphicsQueue)
{
    this->device = device;
...
void vulkanRenderObject::createStageRenderResource(const std::string &objfile, const std::vector<std::string> &pngfiles, const VkDescriptorSetLayout &descriptorSetLayout,const vulkanUniformBuffer &meshUniformBuffer, const vulkanUniformBuffer &viewUniformBuffer,const vulkancubemaperesource &cubemap)
{
    createVertices(object.vertices, object.indices, objfile);
...
/** 从文件中读取顶点和点序*/
void vulkanRenderObject::createVertices(std::vector<Vertex> &outVertices, std::vector<uint32_t> &outIndices, const std::string &filename)
{
    readModelResource(filename, outVertices, outIndices);
}

void vulkanRenderObject::readModelResource(const std::string &filename, std::vector<Vertex> &vertices, std::vector<uint32_t> &indices)
{
...
/** 创建顶点缓存区VBO*/
void vulkanRenderObject::createVertexBuffer()
{
    // 根据vertices大小创建VertexBuffer
    VkDeviceSize bufferSize = sizeof(this->object.vertices[0]) * this->object.vertices.size();
    ...
    memcpy(data, this->object.vertices.data(), (size_t)bufferSize);
...
/** 创建点序缓存区IBO*/
void vulkanRenderObject::createIndexBuffer()
{
...
void vulkanRenderObject::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties, VkBuffer &buffer, VkDeviceMemory &bufferMemory)
{
...
/** 查找内存类型*/
uint32_t vulkanRenderObject::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
/** 通用函数用来拷贝Buffer*/
void vulkanRenderObject::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size)
{
...
VkCommandBuffer vulkanRenderObject::beginSingleTimeCommands()
{
...
void vulkanRenderObject::endSingleTimeCommands(VkCommandBuffer commandBuffer)
{
 ...
 */
void vulkanRenderObject::createDescriptorPool(uint32_t sampler_num)
{
...
void vulkanRenderObject::createDescriptorSets(const VkDescriptorSetLayout &inDescriptorSetLayout,const vulkanUniformBuffer &meshUniformBuffer, const vulkanUniformBuffer &viewUniformBuffer,const vulkancubemaperesource &cubemap)
{
